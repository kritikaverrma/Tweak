...
const getUserProfile = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(404).json({ message: "User not found at server" })
        }
...
const getSuggestedUsers = async (req, res) => {
    try {
        ...
        //find the users followed by me
        const usersFollowedByMe = await User.findById(userId).select("following");

        //scan the entire user collection
        ...
        //exclude those users that follow me back from the all users
        const filteredUsers = users.filter((user) => !usersFollowedByMe.following.includes(user._id))
        //sugget some of those filtered users
